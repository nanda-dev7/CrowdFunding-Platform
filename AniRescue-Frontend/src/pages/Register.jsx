    ...
    <PageTransition className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-7xl items-center gap-10 px-4 py-12 sm:px-6 lg:grid-cols-2 lg:px-8">
      <div className="order-2 lg:order-1">
        <form onSubmit={submit} className="glass rounded-[2rem] border border-white p-6 shadow-soft">
          <div className="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl bg-sage text-coral">
            <PawPrint />
          </div>
          <h2 className="text-2xl font-black tracking-tight text-ink">Create an Account</h2>
          <div className="mt-6 grid gap-4">
            <Input label="First Name" value={values.name} onChange={(event) => setValues({ ...values, name: event.target.value })} required />
          ...
          <p className="mt-5 text-center text-sm text-bark/65">
            Already have an account? <Link className="font-bold text-coral" to="/login">Sign in</Link>
          </p>
        </form>
      </div>
      <div className="order-1 lg:order-2">
        <p className="text-sm font-bold uppercase tracking-widest text-coral">Join the rescue circle</p>
        <h1 className="mt-3 text-5xl font-black tracking-tighter leading-tight text-ink">Small donations can become surgery, medicine, and a safe recovery.</h1>
